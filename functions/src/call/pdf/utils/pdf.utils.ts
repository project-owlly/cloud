...
function generateAmountValidSignatures(doc: PDFKit.PDFDocument, data: any) {
  ...
  doc
    ...
    .fontSize(8)
    .text('Anzahl gültiger Unterschriften', 35, 193);
  //Sig Rechteck
...
function generateHashList(doc: PDFKit.PDFDocument, data: any) {
  ...
  doc
    ...
    .font('fonts/Lato-Black.ttf')
    .fontSize(18)
    .text('Unterschriften IDs', 35, 290);
  //zweikolumnige Liste
  var xAxis = 35;
  var yAxis = 320;
...
function generateAmtsstelle(doc: PDFKit.PDFDocument) {
  doc
    ...
    .font('fonts/Lato-Light.ttf')
    .fontSize(8)
    .text('Bescheinigende Amtsstelle', 35, 742);
...
function generateSkribbleFeld(doc: PDFKit.PDFDocument) {
  ...
  doc
    ...
    .font('fonts/Lato-Light.ttf')
    .fontSize(8)
    .text('Digitale Signatur', 350, 668);
}
...
function generatePDFHeader(doc: PDFKit.PDFDocument, data: any) {
  ...
  if (data.owllyData.type === 'initiative') {
    if (data.owllyData.level === 'canton') {
      doc
        ...
        .fontSize(16)
        .text('KANTONALE VOLKSINITIATIVE', 0, 39.5, {
          align: 'center'
        });
    } else {
      doc
        ...
        .font(`${process.cwd()}/assets/fonts/Lato-Thin.ttf`)
        .fontSize(16)
        .text('EIDGENÖSSISCHE VOLKSINITIATIVE', 0, 39.5, {
          align: 'center'
        });
    }
  ...
  doc
    ...
    .fontSize(30)
    .text(data.owllyData.title, 0, 59.5, {
      align: 'center'
    });
...
function generatePDFStempel(doc: PDFKit.PDFDocument) {
  ...
  doc
    ...
    .fontSize(16)
    .text(format(new Date(), 'dd.MM.yyyy'), 416, 328, {
      align: 'left'
    });
...
function generatePDFDownloadAufruf(doc: PDFKit.PDFDocument) {
  ...
  doc
    ...
    .font(`${process.cwd()}/assets/fonts/Lato-Regular.ttf`)
    .fontSize(6)
    .text('Dokument digital signieren und retournieren an', 0, 430, {
      align: 'center'
    });
  ...
  doc
    ...
    .font(`${process.cwd()}/assets/fonts/Lato-Regular.ttf`)
    .fontSize(6)
    .text('briefkasten@owlly.ch', 0, 442, {
      align: 'center'
    });
}
...
function generatePDFBeschriftungGraueRechtecke(doc: PDFKit.PDFDocument, data: any) {
  ...
  doc
    ...
    .font(`${process.cwd()}/assets/fonts/Lato-Black.ttf`)
    .fontSize(6)
    .text('Wortlaut des Begehrens', 45, 525, {
      align: 'left'
    });
...
function generatePDFFooter(doc: PDFKit.PDFDocument) {
  ...
  doc
    ...
    .font(`${process.cwd()}/assets/fonts/Lato-Light.ttf`)
    .fontSize(9)
    .text('owlly.ch | enabling digital democracy', 0, 812, {
      align: 'center'
    });
}
